...
const PowerConsumer = () => {
  ...
  const filterLogsByTime = (logs, range) => {
    ...
    if (range === "week") {
      ...
      filteredLogs = logs.filter((log) => new Date(log.timeLog) >= oneWeekAgo);
    } else if (range === "month") {
      const oneMonthAgo = new Date(now.setMonth(now.getMonth() - 1));
      filteredLogs = logs.filter((log) => new Date(log.timeLog) >= oneMonthAgo);
    } else if (range === "year") {
      const oneYearAgo = new Date(now.setFullYear(now.getFullYear() - 1));
      filteredLogs = logs.filter((log) => new Date(log.timeLog) >= oneYearAgo);
    }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      getAllSensorLog(`type=${SENSOR_TYPE.AIR}`);
    }, 1800000);
  ...
  useEffect(() => {
    const filteredAccessLogs = filterLogsByTime(sensorLogs, timeRange);
    console.log(filteredAccessLogs);
    const labels = filteredAccessLogs.map((log) => new Date(log.timeLog)); // Lưu ý: bỏ `.toLocaleString()`
    const performanceValues = filteredAccessLogs.map((log) => log.CO2);
  ...
  useEffect(() => {
    ...
    const temperatureValues = filteredSensorLogs.map((log) => log.CO2);
    const humidityValues = filteredSensorLogs.map((log) => log.CO2);
  ...
  return (
    <div className="w-full bg-[#EDEDF5] rounded-[28px] p-[30px] mt-[37px]">
      ...
      <div className="mt-[15px] font-semibold">
        <h2 className="text-[20px]">Biểu đồ khí CO2</h2>
        <div className="mt-[10px] bg-white rounded-[28px]">
          {accessLogs.length > 0 && (
            <Line data={chartDataAccessLog} options={chartOptions} />
          )}
        </div>
      </div>
      {/* <div className="mt-[15px] font-semibold">
